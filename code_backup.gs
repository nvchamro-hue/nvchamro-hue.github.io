...
const SCHEDULE_MONITORING_SHEET = 'ScheduleMonitoring'; // अनुगमन तालिका
...
function doPost(e) {
  try {
    ...
    // Schedule Monitoring action
    if (data.action === 'get_schedules') {
      return handleGetSchedules(data);
    }
    if (data.action === 'update_schedule_status') {
      return handleUpdateScheduleStatus(data);
    }
    if (data.action === 'delete_schedule') {
      return handleDeleteSchedule(data);
    }

    if (data.type === 'schedule-monitoring') {
      var schedSheet = ss.getSheetByName(SCHEDULE_MONITORING_SHEET) || ss.insertSheet(SCHEDULE_MONITORING_SHEET);
      var schedRow = [
        data.editTimestamp || data.timestamp,
        data.sched_type,
        data.sched_pradesh,
        data.sched_jilla,
        data.sched_sthaaniya,
        data.sched_office,
        data.sched_date,
        data.sched_time,
        data.sched_assigned_to,
        data.sched_repeat,
        data.sched_priority,
        data.sched_remark,
        data.sched_status || 'योजना बनाइएको',
        data.sched_completed_date || ''
      ];
      if (data.editTimestamp) {
        updateRowByTimestamp(schedSheet, data.editTimestamp, schedRow);
      } else {
...
function doGet(e) {
  ...
  // Get current user from session parameter
  var currentUser = null;
  if (e && e.parameter && e.parameter.user_session) {
    try {
      currentUser = JSON.parse(e.parameter.user_session);
    } catch (err) {
  ...
  for (var i = 1; i < sRows.length; i++) {
    ...
    if (currentUser && currentUser.role !== 'admin') {
      if (currentUser.role === 'province_admin' && currentUser.province && String(record.pradesh || '').trim() !== String(currentUser.province || '').trim()) continue;
      if (currentUser.role === 'district_admin' && currentUser.district && String(record.jilla || '').trim() !== String(currentUser.district || '').trim()) continue;
      if (currentUser.role === 'user' && currentUser.localLevel && String(record.sthaaniya_taha || '').trim() !== String(currentUser.localLevel || '').trim()) continue;
    }
  ...
  if (mSheet) {
    ...
    for (var j = 1; j < mRows.length; j++) {
      ...
      // Role-based filtering
      if (currentUser && currentUser.role !== 'admin') {
        if (currentUser.role === 'province_admin' && currentUser.province && String(mRecord.m_pradesh || '').trim() !== String(currentUser.province || '').trim()) continue;
        if (currentUser.role === 'district_admin' && currentUser.district && String(mRecord.m_jilla || '').trim() !== String(currentUser.district || '').trim()) continue;
        if (currentUser.role === 'user' && currentUser.localLevel && String(mRecord.m_sthaaniya || '').trim() !== String(currentUser.localLevel || '').trim()) continue;
      }
      
      monitoringData.push(mRecord);
    }
  }
  ...
  if (amSheet && adSheet) {
    ...
    for (var k = 1; k < mRows.length; k++) {
      ...
      // Role-based filtering
      if (currentUser && currentUser.role !== 'admin') {
        if (currentUser.role === 'province_admin' && currentUser.province && String(aRecord.pradesh || '').trim() !== String(currentUser.province || '').trim()) continue;
        if (currentUser.role === 'district_admin' && currentUser.district && String(aRecord.jilla || '').trim() !== String(currentUser.district || '').trim()) continue;
        if (currentUser.role === 'user' && currentUser.localLevel && String(aRecord.sthaaniya || '').trim() !== String(currentUser.localLevel || '').trim()) continue;
      }
      
      attendanceData.push(aRecord);
    }
  }
  ...
  if (pmSheet && pmSheet.getLastRow() > 1) {
    ...
    for (var m = 1; m < pmRows.length; m++) {
      ...
      // Role-based filtering
      if (currentUser && currentUser.role !== 'admin') {
        if (currentUser.role === 'province_admin' && currentUser.province && String(pmRecord.pm_pradesh || '').trim() !== String(currentUser.province || '').trim()) continue;
        if (currentUser.role === 'district_admin' && currentUser.district && String(pmRecord.pm_jilla || '').trim() !== String(currentUser.district || '').trim()) continue;
        if (currentUser.role === 'user' && currentUser.localLevel && String(pmRecord.pm_sthaaniya_taha || '').trim() !== String(currentUser.localLevel || '').trim()) continue;
      }
      
      projectMonitoringData.push(pmRecord);
    }
  }
...
function setupSheets() {
  ...
  // ८. ScheduleMonitoring Sheet सेटअप
  let schedSheet = ss.getSheetByName(SCHEDULE_MONITORING_SHEET);
  if (!schedSheet) {
    schedSheet = ss.insertSheet(SCHEDULE_MONITORING_SHEET);
    schedSheet.appendRow([
      "Timestamp", "अनुगमन प्रकार", "प्रदेश", "जिल्ला", "स्थानीय तह",
      "कार्यालय/आयोजना", "मिति", "समय", "अनुगमनकर्ता",
      "दोहोरिने", "प्राथमिकता", "टिप्पणी", "स्थिति", "पूरा मिति"
    ]);
  }
}
...
function handleGetSchedules(data) {
  try {
    var ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    var schedSheet = ss.getSheetByName(SCHEDULE_MONITORING_SHEET);
    ...
    for (var i = 1; i < rows.length; i++) {
      schedules.push({
        timestamp: rows[i][0],
        sched_type: rows[i][1],
        sched_pradesh: rows[i][2],
        sched_jilla: rows[i][3],
        sched_sthaaniya: rows[i][4],
        sched_office: rows[i][5],
        sched_date: rows[i][6],
        sched_time: rows[i][7],
        sched_assigned_to: rows[i][8],
        sched_repeat: rows[i][9],
        sched_priority: rows[i][10],
        sched_remark: rows[i][11],
        sched_status: rows[i][12],
        sched_completed_date: rows[i][13]
      });
    }
 ...
 */
function handleUpdateScheduleStatus(data) {
  try {
    var ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    var schedSheet = ss.getSheetByName(SCHEDULE_MONITORING_SHEET);
    ...
    var completedDate = data.completed_date || '';

    var rows = schedSheet.getDataRange().getValues();
    for (var i = 1; i < rows.length; i++) {
      if (String(rows[i][0]) === String(ts)) {
        schedSheet.getRange(i + 1, 13).setValue(newStatus);
        if (completedDate) {
          schedSheet.getRange(i + 1, 14).setValue(completedDate);
        }
        return ContentService.createTextOutput(JSON.stringify({
          status: 'success',
...
function handleDeleteSchedule(data) {
  try {
    ...
    var ts = data.timestamp;
    deleteRowByTimestamp(schedSheet, ts);
